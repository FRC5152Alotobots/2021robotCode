...
package frc.robot.components.navigation;
...
public class NavXGyroSubSys extends SubsystemBase {
  /**
  ...
  // NavXGyroSubSys Shuffleboard
    private NavXGyroSubSys_Shuffleboard m_NavXGyroSubSys_Shuffleboard =
    new NavXGyroSubSys_Shuffleboard();

  public NavXGyroSubSys() {
    try {
  ...
  public void periodic() {
    ...
    if(NavXGyroSubSys_Constants.NavXGyroSubSys_Shuffleboard_Enable){
  ...
  public double getGyroAngle(){
    return Math.IEEEremainder(m_NavXGyro.getAngle(), 360) * (NavXGyroSubSys_Constants.GyroReversed ? -1.0 : 1.0);
  }
