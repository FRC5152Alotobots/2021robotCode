...
import frc.robot.components.drive_subsys.swerve_drive.SwerveDriveSubSys_Constants;

import org.opencv.core.RotatedRect;

import edu.wpi.first.math.MathUtil;

public class DriveSubSys_RotateDegreesProfiled_Cmd extends CommandBase {
  
  private final double m_TargetAngleRads;
  private final DriveSubSys m_DriveSubSys;
  ...
  */
  public DriveSubSys_RotateDegreesProfiled_Cmd(double targetAngleDegrees, DriveSubSys driveSubSys) {
    m_DriveSubSys = driveSubSys;
    
    Rotation2d headingChange = new Rotation2d(Units.degreesToRadians(targetAngleDegrees));
    Rotation2d newHeading = m_DriveSubSys.getHeading().plus(headingChange);
    m_TargetAngleRads = newHeading.getRadians();
